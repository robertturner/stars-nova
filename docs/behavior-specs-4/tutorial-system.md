...
### 3a. Tutor and panic-recovery dialogs, confirmed by inspection of the exported client's real dialog resources

The tutorial overlay's persistent window (real caption **"Stars! Tutor"**) exposes exactly three controls: a **Hide** button (dismisses the overlay), a **Hint** button (surfaces contextual help for the current step), and a **Panic!** button. The Panic button opens a distinct, larger dialog with the real caption **"Something's Really Gone Wrong!"**, which offers three recovery choices, each paired with a static line of explanatory text next to it:

- **"Redo Turn"** — explanatory text: "This button wipes the slate and restarts the turn with none of your changes."
- **"Complete Turn"** — explanatory text: "This button restarts the turn and completes all the required tasks for you. If you get stuck, this will do everything for you and allow you to view the results."
- **"Never Mind"** (the dialog's default/cancel-equivalent choice) — explanatory text: "This button dismisses this dialog without changing anything."

A fourth static line documents the dialog's own **Help** button: "This button will bring up context sensitive help on the task you are stuck on. This button does the same thing as the Tutor's help button." This confirms the panic dialog is a tutorial-only escape hatch (an automated "do the rest of this turn for me" recovery path) rather than a general error-recovery surface, and that its three choices are the complete set — no other recovery options exist.
