...
namespace Nova.Ai
{
    ...
    public class DefaultPlanetAI
    {
        ...
        /// <summary>
        /// ai-opponent-behavior.md section 6's own stated hard structural limit: a build-order is
        /// refused outright once a planet's queued-item count exceeds this, independent of the
        /// per-item 1,023-unit quantity cap documented in production-queue.md.
        /// </summary>
        private const int MaxQueuedProductionItems = 200;
        ...
        public DefaultPlanetAI(Star newStar, ClientData newState, DefaultAIPlanner newAIPlan, Random random)
        {
            planet = newStar;
        ...
        /// <summary>True while this planet's production queue still has room for another
        /// item under the spec's 200-item cap (see MaxQueuedProductionItems).</summary>
        private bool QueueHasCapacity()
        {
            return this.planet.ManufacturingQueue.Queue.Count < MaxQueuedProductionItems;
        }
        ...
        public void HandleProduction()
        {
            ...
            if (this.planet.ResourcesOnHand.Germanium > 50)
            {
                ...
                if (factoriesToBuild > 0 && QueueHasCapacity())
                {
            ...
            if (this.planet.Mines < maxMines && QueueHasCapacity())
            {
            ...
            // build defenses (see DefensePercentageAdvisor for the spec/approximation this ports)
            int defenseToBuild = Global.MaxDefenses - this.planet.Defenses;
            int actChancePercent = DefensePercentageAdvisor.ActChancePercent(this.planet.Defenses, Global.MaxDefenses);

            if (random.Next(100) < actChancePercent && defenseToBuild > 0 && QueueHasCapacity())
            {
