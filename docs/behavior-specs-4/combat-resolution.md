...
- **The accuracy formula (§6, Open Questions) has new partial evidence, not a full resolution.** A function was found that simulates each individual missile/torpedo's hit/miss check via an independent bounded random roll against a computed percentage threshold — directly confirming §6's "each missile/torpedo is resolved as an independent hit/miss check" claim at the code level. That threshold is derived from a base value and a target-jamming input through a formula of the shape `adjusted = (100 − jam) × (base − 100) / 100 + 100` when jamming is present, otherwise a simpler proportional reduction — a concrete candidate for the still-unconfirmed exact accuracy/jamming formula, but it was not cross-checked against the community-sourced numeric table in §6 (e.g. the 20%/44%/Jammer-20 worked example), so it is offered here as new evidence toward closing that Open Question rather than as a final answer.
- **The armor-quantization scheme (§6) was located but its exact unit could not be matched to the documented 1/512 figure.** The same damage-application function tracks a token's cumulative armor damage in a sub-byte fractional field (masked to 7 bits, i.e. a 0–127 range) rather than the 1/512 (9-bit) granularity this document's §6 states from community sources; this may be a different field serving a related purpose (e.g. a separate "wear" or shield-regen-adjacent value) rather than the same mechanism, and is flagged here rather than used to silently correct §6's 1/512 claim.
