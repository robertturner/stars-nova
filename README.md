...
## Fleet Split suggests the source fleet's name; map switcher updates immediately (2026-09-16)

Two small fixes to the Inspector's Split/Merge tab and the mobile map's fleet/star switcher:

**Suggested split-off name.** Splitting a fleet always named the new fleet the engine's own
generic "New Fleet #N" (`EmpireData.MakeNewFleet`), leaving the player to manually rename it every
time - even for the common case of splitting off part of a fleet while keeping both halves under
the same name (e.g. peeling one ship off a squadron). The Split/Merge tab now has an editable "New
fleet's name" field, defaulted to the source fleet's own name and only shown when the "Other side"
target is "New Fleet" (renaming an existing merge target makes no sense). `ApplySplitMerge` applies
it via a `RenameFleetCommand` against the newly-created fleet once it's actually in `OwnedFleets`,
after the split itself completes - left as the engine default if the field is cleared entirely.

**Stale map switcher after a Split.** `MapSelectionSwitcherViewModel` (the "Viewing" dropdown that
lets the mobile Map screen switch between a star and any of the player's own fleets in orbit there)
only rebuilt its option list when the anchor star/planet itself changed reference - a deliberate
optimization so merely switching between the star and one of its own already-listed fleets wouldn't
visibly rebuild the list. But that meant an in-place mutation that leaves the anchor unchanged while
changing who's actually in orbit - e.g. a Split creating a brand-new fleet at the same star - left
the dropdown stale until the player navigated away and back (the only way to force the anchor
reference itself to change). Fixed by always recomputing the option list on every refresh, but only
actually reassigning it (and so rebinding/flickering the ComboBox) when the recomputed list differs
from the current one - preserving the original optimization's UX while fixing the staleness.

**Verified live** on the `Nova_Test` emulator: merged Scout #1 and Santa Maria #1 (both single-ship
fleets, so a real split required first building a multi-design fleet to split back apart) into one
2-design fleet, then split the Santa Maria design back off. The "New fleet's name" field correctly
pre-filled "Scout #1" (the source fleet's name) at every step; applying the split immediately showed
all three entries - "Nova (Planet)", "Scout #1" (original, kept the Scout), and "Scout #1" (newly
split-off, holding Santa Maria) - in the Viewing dropdown with no navigation away and back needed.
Full suite unaffected (176/178, same 2 pre-existing failures) - no new unit test: both fixes are
Inspector/ViewModel wiring (a rename command already covered by `RenameFleetCommand`'s own use
elsewhere, and a pure list-diffing change to existing selection-sync logic) with no new domain logic
of their own to cover.

## Star Habitability shows a projected post-terraform value; fixed a replacement starbase lingering as a stray fleet (2026-09-16)

Two unrelated fixes:

**Terraform projection.** The Inspector's owned-star Habitability row only ever showed the
CURRENT value (e.g. "58%"), with no way to tell whether terraforming could still meaningfully
improve it. Added `Race.HabitalValueAfterTerraform(Star)`, which nudges each of Gravity/
Temperature/Radiation toward this race's own optimum by up to its total terraform allowance (15%,
or 30% with Total Terraforming) measured from the star's ORIGINAL (pre-terraform) value - the same
ceiling `TerraformProductionUnit`'s own turn-by-turn 1% steps are capped by, computed directly
rather than stepped through. `ShowStar` now shows "58% (72%)" whenever the projected value would
actually be higher than the current one, and just "58%" (no redundant, identical parenthetical)
once a star is already at or beyond this race's terraform ceiling.

**Stray fleet from a starbase upgrade.** `Manufacture.CreateShips`'s starbase-replacement branch
only ever did `star.Starbase = null` before assigning the newly-built one - detaching the OLD
starbase fleet from the star without removing it from the empire's own `OwnedFleets`/
`FleetReports` bookkeeping (`AddOrUpdateFleet` had already unconditionally added it when it was
itself first built). Left dangling there with nothing pointing back to it, the old starbase kept
showing up everywhere "the fleets in orbit at this star" are listed (e.g. the mobile Map's
"Viewing" switcher) even though the Inspector's own Overview correctly showed the new one as the
star's starbase. Fixed by actually calling `EmpireData.RemoveFleet` on the old starbase fleet
before replacing the reference.

**Verified**: the terraform projection has four new `StarTest` unit tests (headroom available,
Total Terraforming doubling the allowance, no headroom left, already at optimum) plus a live check
on the `Nova_Test` emulator confirming both owned stars (already at 100% habitability) correctly
show no redundant parenthetical. The starbase fix has a new `ManufactureStarbaseReplacementTest`
that builds a starbase, then a replacement one at the same star, and asserts the original is gone
from `OwnedFleets` - deliberately reverted the fix and confirmed the test fails without it before
restoring it, since this is a turn-processing/server-side bug that's impractical to reproduce live
in this environment (needs a real turn to advance, known slow/unreliable with this save's AI).
Full suite: 181/183 (176/178 plus these 5 new tests), same 2 pre-existing unrelated failures.

## Existing ship designs are now viewable via press-and-hold, from the Ship Design page and the Navigator's Fleet view (2026-09-16)

The shared read-only HullViewer (already used from the Star Map's fleet markers and the
Inspector's own "Ships" composition list) wasn't reachable from two other places a player would
naturally want to inspect a design's component layout:

- **Ship Design page's "My Designs" list** - press-and-hold a design row now shows its full
  component layout, same as everywhere else. Built as a `ListBox` (mirroring the Navigator's own
  Fleets list), confirmed working via a mid-hold screenshot (`SelectedItem` is deliberately left
  unbound - this list has no selection concept, only Delete/hold-to-inspect). Note: an earlier
  version of this entry claimed a plain `ItemsControl` couldn't fire `Holding` at all inside a
  ScrollViewer - that was wrong, an artifact of screenshotting only *after* the hold gesture had
  already finished (see the "Verified live" note below); the `ListBox` shape was kept anyway since
  it's confirmed correct, not because the alternative was actually shown to be broken.
- **Navigator's Fleets list** - previously only ever showed/exposed the fleet's FIRST ship design
  (`Fleet.Icon`/`PrimaryDesign` - "choose an image from one of the ships"), so a multi-design fleet
  (e.g. an escorted colonizer) could never have any type but that one inspected here. Each fleet
  row now shows one icon per DISTINCT design in its composition
  (`NavigatorFleetItemViewModel.ShipTypes`, backed by a new `NavigatorFleetShipTypeViewModel`),
  each independently press-and-holdable to show ITS OWN layout.

**Verified live** on the `Nova_Test` emulator, including tracking down a false alarm: an initial
pass concluded the Ship Design page's hold gesture (and even the pre-existing, untouched Inspector
"Ships" list one, itself a plain `ItemsControl`) simply didn't fire at all, since a screenshot
taken shortly *after* the press-and-hold input command finished never showed the popup. Capturing
a screenshot *during* the hold (backgrounding a long `adb shell input swipe` and screenshotting
partway through) proved both actually work fine - the popup opens and closes correctly within the
gesture's own lifetime, just faster than a post-release screenshot could catch. That means the
`ListBox` conversion on the Ship Design page was never actually shown to be NECESSARY (a plain
`ItemsControl`, tested correctly, likely would have worked here too, going by the Inspector's own
precedent) - only that the `ListBox` shape it ended up with is itself confirmed correct. No new
unit test: this is Avalonia view/gesture wiring with no domain logic of its own to cover.

## Saves with an already-orphaned starbase fleet now self-heal on load (2026-09-17)

A user reported a save file where a star's (player-built, replacement) starbase showed up as a
stray extra fleet, on a build from before the previous entry's `Manufacture.CreateShips` fix. That
fix only stops the corruption from happening to a NEW starbase replacement going forward - it does
nothing for a save that already has the orphaned old starbase fleet baked into its `OwnedFleets`/
`FleetReports` from before the fix shipped, since that data was already written to disk by the
time the bug was fixed.

Rather than writing a one-off repair script for this one save, `EmpireData.LinkReferences()` (run
on every load of a save - client and server alike, since `ServerData` itself loads each empire via
the exact same XML constructor) now calls a new `RemoveOrphanedStarbaseFleets()`: any owned,
Starbase-type fleet sitting in orbit at a star this empire owns, but which isn't that star's own
(already-resolved) `Starbase` reference, is exactly that leftover corruption - a star can only
ever have one real starbase, so anything else matching this shape is never legitimate - and gets
removed via the existing `RemoveFleet` (clearing both `OwnedFleets` and `FleetReports`). Any
already-affected save self-heals the moment it's next opened or processed, with no separate repair
step needed.

Investigated, but did not find a cause for, the second symptom reported alongside the stray
fleet - that star's population no longer growing turn over turn since building the replacement
starbase. Read through the full growth pipeline (`Star.CalculateGrowth`/`UpdatePopulation`,
`StarUpdateStep`) and every turn step that touches a star's own fleets (`ScanStep`, `Bombing`,
`BattleEngine`'s co-location grouping, fleet movement/regeneration) - none of it references
`OwnedFleets`/`InOrbit` in a way an extra co-located fleet could plausibly confuse, and nothing in
that pipeline runs inside a try/catch that could be silently swallowing an exception the phantom
fleet triggers elsewhere first. If the symptom persists once the save has gone through the repair
above, it's something else entirely and needs its own investigation with the actual save file.

**Verified**: two new `EmpireDataOrphanedStarbaseRepairTest` cases - one builds an empire with both
an orphaned old starbase and a real current one (the exact shape a pre-fix save would have saved),
round-trips it through `ToXml()`/reload, and confirms only the orphan is gone; the other confirms a
genuine, never-replaced starbase is never mistaken for one and removed. Deliberately reverted the
fix and confirmed the first test fails without it before restoring it. Full suite: 183/185 (same 2
pre-existing unrelated failures).

## Widened tap/hold targets on the Star Map and in icon-based hold-to-inspect lists (2026-09-17)

Both selecting a star/fleet on the map and holding an icon in the Navigator/Inspector/Ship Design
lists to inspect its hull required tapping very precisely - especially on the map, where every
marker's on-screen size shrinks along with the whole map's `LayoutTransformControl` zoom, so a
zoomed-out map made already-small markers even harder to hit.

- **Star Map markers**: the star marker's tap target grew from 16x16 to 32x32, and the fleet
  marker's from 14x14 to 32x32, both still centered on the same origin point via the same
  `Canvas.Left`/`Top` offset trick - the visible dot/triangle stays exactly the same size and
  position, only the invisible surrounding hit area got bigger.
- **Navigator's ship-type icons, Inspector's composition-row icons, Ship Design's "My Designs"
  rows**: each hold target's `Border` grew from a few pixels beyond its actual icon (28x28→40x40,
  24x24→36x36) or already spanned most of the row (My Designs).

Widening the icon `Border`s surfaced a real, previously-invisible gotcha: a `Border` with no
`Background` set isn't hit-test visible in its own empty space - only wherever its child actually
paints something. Every existing hold gesture happened to work because testing always (by luck)
landed dead-center on the icon/text itself; the moment the *widened, otherwise-empty* margin
was tested, holding there silently did nothing until `Background="Transparent"` was added to each
`Border` - confirmed both ways live. This also means the previous session's already-existing hold
targets (before today) always relied on hitting exact pixel content, not "somewhere near it" as
their nominal Border size implied - this fix is what actually delivers the wider tap tolerance,
not just the size bump on its own.

**Verified live** on the `Nova_Test` emulator at 57% map zoom: a tap 7px off Rye's exact dot
(previously well outside its ~4.5px-radius hit circle) now correctly selects it, while a tap 10px
off still correctly misses (consistent with the new ~9px-radius box, not a wide-open false
positive). Holding in the blank margin around the Navigator/Inspector icons and in the blank space
to the right of a short design name on the Ship Design page - all previously dead space - now
correctly opens the HullViewer. No new unit test: this is pure Avalonia layout/hit-testing, with
no domain logic of its own to cover.

## Merging two fleets defaults to moving everything, not nothing; added a "Share Save File" menu entry; fixed the burger menu overflowing off-screen (2026-09-17)

**Merge discoverability.** A user reported "split works, but can't see how to merge 2 fleets" - the
underlying mechanism (Split/Merge tab, "Other side" picker, per-design sliders, Apply) already
worked for a genuine merge (picking an existing fleet as the target, moving ships to it), but every
row's slider always defaulted to "Keep: everything, move nothing" regardless of whether the target
was a brand-new fleet (a split, where that default makes sense) or an existing one (a merge, where
the user almost always wants to move everything there). With no visible difference between the two
cases, picking an existing fleet from the dropdown and tapping Apply immediately just did nothing
("Nothing to move" - true, since nothing had moved) with no obvious next step. `SelectedSplitMergeTarget`'s
setter now re-defaults every row's `KeepInSource` based on which kind of target was just picked -
0 (move everything) for an existing fleet, `OriginalQuantity` (move nothing) for "New Fleet" - so
the common case of either action needs no slider fiddling at all, while a partial merge/split is
still just a drag away.

**"Share Save File".** Investigating a separate report (a starbase-replacement fleet still showing
as a stray fleet even after the self-heal fix from earlier today) needed the user's actual save
file to diagnose - but Android's scoped storage means a save living in the app's own private
storage has no plain file-browser access, and `adb run-as` (the usual workaround) refuses on a
release build that isn't debuggable. Added a `PlatformHooks.ShareText` hook (generalizing
`ShareErrorLog`'s existing Android `ActionSend` share-sheet mechanism, which already proved this
exact approach works for arbitrary text without needing a FileProvider/file attachment at all) and
a new "Share Save File" burger-menu entry that reads the currently-open game's own .intel file
(plain XML - fleets, stars, starbase references, everything) and offers it through the native
Share sheet - Drive, email, Messages, whatever's on the device - the only practical way to get a
copy off an Android device for a release build.

**Burger menu overflow.** Adding that entry surfaced a real, pre-existing bug: the burger menu's
item list was never wrapped in a `ScrollViewer` - its containing Grid row is "Auto"-sized (grows to
fit all content) with nothing capping or scrolling it, so once enough entries piled up, the last
few (now "Share Save File" and "Close Game") ran off the bottom of the screen with no way to reach
them at all on a short device - confirmed live: tapping where "Share Save File" should have been
did nothing, since Android's own accessibility tree marked it not visible / partially outside the
window. Fixed by wrapping the menu's content in a `ScrollViewer` with a generous `MaxHeight` (420),
so it scrolls internally instead of overflowing - confirmed live afterward with a visible scrollbar
and every entry reachable.

**Verified live** on the `Nova_Test` emulator: merging Scout #1 into Santa Maria #1 now defaults
every composition row to "move everything" the moment Santa Maria #1 is picked from "Other side",
requiring only a tap on Apply for the common case; "Share Save File" opens the native Share sheet
showing the correct .intel XML content; the burger menu now scrolls, with every entry (including
the two new-since-today ones) reachable regardless of screen height. Full suite unaffected
(183/185, same 2 pre-existing failures) - no new unit test: all three are Avalonia
view/gesture/platform-hook wiring with no domain logic of their own to cover.

## Root-caused and fixed the starbase-as-stray-fleet bug for real, using the user's actual save (2026-09-17)

The self-heal from earlier today didn't actually fix the reporting user's save, so they used the
new "Share Save File" feature to send its real .intel content - which showed exactly why.

**The real root cause**: `StarMapInitialiser.AllocateStarbase` - which builds a game's very FIRST
starbase at creation time (every home star, plus Interstellar Traveler/Packet Physics' second
starting planet) - correctly wired `star.Starbase` and correctly added the fleet to
`OwnedFleets`, but never stamped the fleet's own `Type` to `ItemType.Starbase` (the
`Fleet(ShipToken, Star, long)` constructor defaults it to plain `ItemType.Fleet`, and nothing here
ever corrected that). `Manufacture.CreateShips`'s starbase branch, by contrast, always gets this
right. So the exact bug reported earlier - a replacement starbase's predecessor lingering as a
stray fleet - was real, but the self-heal's own detection (`fleet.Type == ItemType.Starbase`)
could never recognize a game's ORIGINAL starbase as one in the first place, since that fleet's
Type was never Starbase to begin with. Confirmed directly from the user's save: Rotanev's original
"Stargate"-design starbase sat in `OwnedFleets` with `Type=Fleet`, next to its real replacement
(a "Space Station mini" design, correctly `Type=Starbase`) - and Nelson's still-original,
never-replaced starbase had the exact same `Type=Fleet` mis-tagging, just harmlessly, since it was
still the star's own live Starbase and had nothing to be orphaned from (yet).

**Two fixes**: `AllocateStarbase` now stamps `Type = ItemType.Starbase` on a starting starbase, so
future games no longer create this corruption in the first place. `EmpireData.RemoveOrphanedStarbaseFleets`
now judges "starbase-shaped" from the fleet's own COMPOSITION (its design's `Type`) rather than
trusting the fleet's own (possibly-never-set) `Type` field - `IsStarbaseShaped` checks
`fleet.Type == ItemType.Starbase` OR the composition's design `Type == ItemType.Starbase`, so it
catches a mis-typed original starbase too, not just a correctly-typed Manufacture-built one.

**Verified**: a new `EmpireDataOrphanedStarbaseRepairTest` case reproduces the exact real-world
shape (an original starbase with `Type` left at the constructor default, later superseded by a
correctly-typed replacement) and confirms the mis-typed original is still recognized and removed.
Also directly verified against a faithful excerpt of the user's own actual save data (loaded via
the real `EmpireData` XML constructor, not just a synthetic fixture) - confirmed the fix reduces
Rotanev's `OwnedFleets` entries from 2 down to the correct 1, keeping the real current starbase and
removing the mis-typed original. Full suite: 184/186 (same 2 pre-existing unrelated failures).

## Fleet rename from Split/Merge (plus a real map-switcher staleness bug it exposed); Ship Design locks an engine slot's quantity to the hull's requirement; a real population-growth boundary bug (2026-09-17)

**Split/Merge fleet rename.** The user asked for the Split/Merge tab to also allow renaming a
fleet, reusing the "New fleet's name" textbox. That box was previously only visible when the
target was "New Fleet" (a split); it's now always visible, with its label switching between
"New fleet's name" (splitting) and "Rename this fleet" (merging/no-op) via a new
`SplitMergeNameFieldLabel` computed property. `ApplySplitMerge()` gained a pure-rename path
(`wantsSourceRename`: target isn't "New Fleet" and the name actually differs from the fleet's
current one) that applies a plain `RenameFleetCommand` even when nothing is being moved - previously
any attempt with zero ships moved was unconditionally rejected as "Nothing to move", which blocked
a rename-only Apply outright. A rename typed alongside a genuine split/merge still works too: the
existing post-merge rename-the-new-fleet branch gained a sibling that renames the SOURCE fleet
instead, for the case where nothing new was created (a merge empties the source into an existing
fleet, or a split moves everything out) but the source fleet itself survives and the user typed a
new name for it.

**`MapSelectionSwitcherViewModel` staleness bug (found during verification).** Renaming a fleet and
confirming it live surfaced a second, unrelated bug: the mobile Map screen's "Viewing" dropdown
(switches between a star and its co-located fleets) kept showing a fleet's OLD name after renaming
it, until navigating away and back. `RebuildOptions()` already had a "only reassign Options if it
genuinely changed" optimization to avoid needless ComboBox rebinding, but it compared only
`Selectable` object identity - a rename changes a fleet's `Name` string while leaving the same
`Fleet` object in place, which that comparison couldn't see. Fixed by comparing `(Selectable, Name)`
pairs instead of `Selectable` alone.

**Ship Design engine-count lock.** The user asked that Ship Design auto-set a hull's engine count
to the number of engine mounts the hull requires, and not allow reducing it - unlike a
Weapon/Armor slot, where filling fewer than the maximum is a legitimate cost/mass tradeoff, a
multi-engine hull running under-mounted isn't a real reduced-power option, just a broken design.
Added `HullSlotRowViewModel.HasFixedQuantity` (`Module.ComponentType == "Engine"`, matching the
same check already used in `StarMapInitialiser`/`DefaultAIPlanner`). The `Quantity` setter now
forces a filled engine slot's requested value up to `Module.ComponentMaximum` regardless of what
was actually asked for, before clamping - covering both the Decrement button AND `TryPlace`'s own
drag-drop math, since a drop can request any count directly. `DecrementCommand.CanExecute` also
gained `&& !HasFixedQuantity` so the button itself visibly disables. `Clear()` is unaffected and
still correctly empties an engine slot to 0, since it nulls `Module.AllocatedComponent` (making
`IsFilled` false) before setting `Quantity = 0`, and the lock only applies while `IsFilled` is true.

**Verification note (was an honest gap, now closed - see below):** the logic here is straightforward
and directly mirrors the already-proven generic slot-quantity clamping/CanExecute pattern already
live elsewhere in this same file. Live UI verification of the "switch hulls and fit an engine" step
was blocked at the time by an unrelated Ship Design page interaction bug (below) - now root-caused,
fixed, and re-verified live: filling an empty engine slot auto-fits to that slot's max, and the
Decrement button correctly does nothing on a filled engine slot (confirmed live on a 1-engine
Privateer hull - no 2+-engine hull was unlocked by this race's current tech to exercise the
"auto-fills to 2+" case specifically, but it's the exact same code path off `Module.ComponentMaximum`,
just a different value, so the 1-engine case fully exercises the actual logic being verified).

## Root-caused and fixed the Ship Design page's "tap does nothing, works on retry" bug (2026-09-17)

The user asked directly: "How do we sort the Ship Design page scroll/ComboBox interaction?" -
picking back up the gap flagged above. Reproduced it live on the `Nova_Test` emulator (after
working around an unrelated environment snag - a Release APK installed via a raw `adb install`
hung indefinitely at the splash screen on this session's fresh emulator boot, while the same build
installed via the established `dotnet build -t:Install` MSBuild target launched normally; this
looks like a deployment-path quirk specific to sideloading a Release build outside the normal
Debug-install workflow, not an app bug, and didn't recur once the proven install method was used).

**Root cause**: Avalonia's `ScrollViewer` retains scroll-gesture ownership for a period *after* a
touch-scroll's finger-up, while its fling/momentum deceleration plays out - not merely "some
residual animation," but an actual multi-second window where the *next* pointer-down is still
arbitrated as continuing/settling that scroll rather than being free to reach a child control's own
Click handler. Confirmed directly: tapping the Hull `ComboBox` immediately after scrolling it into
view produced a tiny, silent scroll adjustment and no dropdown, every time; tapping the exact same
spot again a moment later (or after deliberately waiting ~2-3 seconds for the fling to fully decay)
opened it correctly on the first try. This explains the earlier session's specific symptom pattern
precisely - "the Hull ComboBox and items near a scroll boundary" were never uniquely broken; ANY
control tapped in the natural "scroll to reveal it, then immediately tap it" sequence hits this
window, while a control that happened to get tapped a beat later (or via an incidental second tap)
looked fine by comparison.

**Fix**: `ScrollViewer.IsScrollInertiaEnabled="False"` on Ship Design's "everything else" ScrollViewer
([ShipDesignView.axaml](Nova.Avalonia/Views/Panels/ShipDesignView.axaml)) - a real Avalonia 12.1.2
API (`Avalonia.Controls.ScrollViewer.IsScrollInertiaEnabled`, "whether scroll gestures should
include inertia in their behavior and value"). Removing inertia means a touch-drag now scrolls by
close to its own raw drag distance with no added fling-carry, which trades away "flick to scroll
far" for a scroll that comes to rest the instant the finger lifts - so the very next tap is never
contested. **Verified live, repeatedly**: scrolled to the Hull ComboBox and tapped it with zero
delay (the exact original failure sequence) - opened correctly on the first tap, several times in a
row, including immediately after small scroll adjustments. Also used this fixed build to complete
the engine-lock verification noted above. Only applied to this one ScrollViewer for now (the one
actually reported); the same fix likely applies anywhere else in the app a tappable control sits
inside a touch-scrollable region, worth a pass if the same symptom is reported elsewhere.

**Build/test**: full suite unaffected (187/187) - this is a pure Avalonia view property change with
no unit-testable logic of its own.

**Population growth boundary bug.** The user reported Rotanev's population not growing despite 100%
habitability, in the same save shared for the starbase bug above. `Star.CalculateGrowth`'s "low pop"
and "early crowding" branches were `capacity < 0.25` and `capacity > 0.25 && capacity < 1.0` - both
excluding `capacity == 0.25` exactly, which silently fell through to the "full planet" branch (zero
growth) whenever a colony's capacity rounded to precisely 25%. `Capacity()` returns a rounded-up
integer percentage (`Math.Ceiling`), so a real colony sitting anywhere in roughly 24.01%-25.00% true
capacity rounds to exactly 25 and hits this gap - which is exactly what Rotanev had. The project's
own `docs/behavior-specs-4/population-growth.md` §3, sourced from starsfaq.com, states the no-crowding
case as an unconditional `capPct <= 0.25` - the original code's strict `<` didn't match its own cited
source. Fixed to `capacity <= 0.25` (low pop, now inclusive) with the early-crowding branch simplified
to `capacity < 1.0` (the else-if chain makes the lower bound implicit). Added
`StarTest.LowPopGrowth_AtExactlyTwentyFivePercentCapacity`, confirmed failing without the fix (via
`git stash` on just `Star.cs`) and passing with it restored.

**Build/test**: `dotnet build Nova.sln` clean (0 errors). Full suite: 185/187 (same 2 pre-existing
unrelated failures: `Test4SelectTargets`, `calculateAdvantagePointsForStandardJoat`), covering all
four fixes above combined.

## Fixed both long-standing pre-existing test failures for real, instead of continuing to carry them (2026-09-17)

Investigated the "same 2 pre-existing failures" this session had been carrying forward in every
changelog entry above, rather than assuming they were unfixable - both turned out to be real,
fixable bugs in the tests themselves, not in the production code they exercise.

**`calculateAdvantagePointsForStandardJoat`** asserted a standard JOAT race scores 25 advantage
points; the calculator actually (and consistently) produces 29. `git log` on
`RaceAdvantagePointCalculator.cs` explains why: commit `ecfb1cd` ("Bug: Race Designer Costs -
issue #3296768 ... **standard joat has now 29 points**") deliberately changed the calculation to
fix that SourceForge-reported bug against the real game's known value, but the test's hardcoded
expected constant was never updated to match - it's been asserting the pre-fix, buggy value for
over a decade. Updated the test's expectation to 29.

**`Test4SelectTargets`** (`BattleEngineTest`) asserted that two enemy stacks, mutually in range,
each pick the other as a target - but only one direction ever did, non-deterministically depending
on unrelated factors elsewhere in a full test run (it reliably passed in isolation, which is why
it looked "flaky" rather than broken). Root cause: `ShipDesign.Cost` reads a `Summary` field that
starts out zeroed and is only ever populated by calling `Update()` - a lazy-recompute pattern this
codebase already has THREE other tests carrying an explicit comment for
(`ColonizerCommitmentAdvisorTest`: "CargoCapacity reads Summary, which only Update() populates").
`BattleEngineTest`'s hand-built `cruiser`/`frigate` designs never called `Update()` at all. In real
gameplay this never manifests - `EmpireData.LinkReferences()` calls `Update()` on every owned
design on every load, long before any battle could occur - but in this test, `BattleEngine.
SelectTargets`'s `GetAttractiveness` ended up reading a target's Cost as 0 whenever that target's
own design hadn't separately had its `IsArmed` checked yet as an attacker in the same loop
(`SelectTargets` only ever calls `Update()`, indirectly via `IsArmed`, on the current attacker's
own design, never on the design being evaluated as a target) - so whichever stack's design
happened to get "warmed" first (an accident of iteration order, not something the test itself
controls) was the only one to correctly evaluate the OTHER stack as attractive; the other
direction saw a stale zero cost and picked no target at all. Fixed by calling `cruiser.Update()`/
`frigate.Update()` right after building each design in the test's constructor, matching the
established pattern elsewhere in this suite.

**Full suite is now clean: 187/187**, confirmed across 3 consecutive full runs with no flakiness -
no more "same N pre-existing failures" caveat needed going forward.

## Implemented the per-PRT starting fleet system - a decade-old dead `switch` finally wired up (2026-09-17)

A user noticed their Interstellar Traveler game only had a Scout at the primary home star, not
the second Stargate-equipped planet. That's correct as far as it goes (see the earlier "Star
Habitability shows a projected post-terraform value" entry, 2026-09-16, which implemented the
second planet itself) - but investigating it surfaced the deeper gap `docs/behavior-specs-4/
race-traits.md` already had flagged: `StarMapInitialiser.cs`'s `PrepareDesigns` had an entire
second `switch (race.Traits.Primary.Code)` - distinct from `Gameinitializer.ProcessPrimaryTraits`'
own real, working switch that sets starting RESEARCH levels - wrapped in a literal `/* ... */`
block comment, describing what bonus SHIPS each Primary Trait should start with. It had never
executed. Every race started with exactly the same one scout + one colony ship + one starbase
(three colony ships for Hyper Expansion specifically, and Packet Physics/Interstellar Traveler's
second planet, both already implemented separately) regardless of PRT.

**Sourced for real this time.** The dead comment's own one-line-per-PRT guesses were never cited to
anything. Found the actual official Stars! Player's Guide PDF on archive.org
(https://archive.org/download/manual_Stars/Stars.pdf) and extracted its text with `pdftotext` (no
readable text extraction tool otherwise available in this environment) - Step 2: Primary Trait (pp
20-3 to 20-11) has a "Starting Advantages" bullet list for every single PRT, giving exact ship
counts. This confirmed most of the old comment's guesses (Hyper Expansion/War Monger's armed
scout, Space Demolition's two mine layers, Interstellar Traveler's destroyer+privateer, Jack Of All
Trades' full varied fleet) and clarified the rest (Packet Physics gets TWO shielded scouts, not one
plain one; Claim Adjuster's "orbital terraforming ship" is a Scout fitted with an Orbital Adjuster,
per a separate manual passage: "every race with the Claim Adjuster trait starts out with one ship
outfitted with Orbital Adjusters"). Super Stealth, Inner Strength and Alternate Reality all get
nothing extra - confirmed by the manual explicitly restating just the universal baseline for the
first two, and omitting a "Starting Advantages" section entirely for AR (which already gets its own
distinct colonizer component, implemented separately, long before this fix).

**Implementation** (`StarMapInitialiser.cs`): replaced the dead comment with real per-PRT design
construction (an armed Scout variant for HE/WM; a shielded Scout variant for PP; a Scout fitted
with an Orbital Adjuster for CA - which needs Biotechnology 6, already granted by
`ProcessPrimaryTraits` before this code ever runs; two Mini-Mine-Layer designs for SD, one with
Mine Dispenser 40 and one with Speed Trap 20 - which needs Biotechnology 2 and Propulsion 2,
exactly SD's existing tech bonus, presumably why those two fields were chosen for it originally; a
shared Destroyer design for IT/JOAT; a Privateer for IT; a Medium Freighter and Mini Miner for
JOAT), plus new `FindDesign`/`AddShipFleet` helpers and an `AllocateBonusStartingShips` method to
attach the right combination of fleets per PRT at the primary home star only (not IT/PP's second
planet, matching the manual's one-off wording).

**Two real, previously-latent bugs found and fixed along the way, both pre-existing and unrelated
to the new feature itself:**
- **Shared-hull corruption**: building two designs from one `AllComponents.Fetch(hullName)` call
  (reusing the same already-fetched `Component` for two `ShipDesign.Blueprint`s) silently makes
  them share the same underlying `Hull.Modules` list - `Fetch()` deep-clones per call, but a plain
  `Blueprint = someAlreadyFetchedComponent` assignment doesn't clone anything, so the SECOND
  design's module-filling loop overwrites the FIRST design's slots too. Hit this twice while
  writing the new code (Scout-hull variants sharing the pre-existing "Scout" design's hull; SD's
  two mine-layer designs sharing each other's) - fixed by calling `Fetch()` once per design, never
  reusing an already-built one.
- **`ShipDesign.Update()`'s `HeavyMines`/`SpeedBumbMines` accumulators started from `MineLayer`'s
  class-default `HitChance` (0.3, i.e. "Standard") instead of their own bucket's real value** (1.0
  for Heavy, 3.5 for Speed Trap) - `MineLayer.operator+` rejects adding two different-HitChance
  layers together (by design, "this only makes sense if the mine layers produce the same mines"),
  so the very FIRST heavy or speed-trap mine-layer component ever summed into a fresh accumulator
  always mismatched and got silently rejected (logging "MineLayer.operator+ Attempted to add
  together different types of mine layers" and leaving the accumulator at zero) - a design's Heavy
  or Speed Trap laying rate could never become nonzero, anywhere in the app, for any race. Only
  `StandardMines` ever worked, purely because Standard's own real HitChance (0.3) happens to equal
  that unrelated class default. Fixed by seeding each accumulator with its own correct starting
  `HitChance` in `Update()`'s reset block.

**Verified**: 8 new integration tests in `NewGameTest.cs` (one, or one `TestCase` pair, per PRT
bonus), checking the actual fleets/designs `GeneratePlayerAssets()` produces - including that SS/IS/AR
get nothing extra beyond the universal baseline. `docs/behavior-specs-4/race-traits.md`'s own "open
empirical question" entry for this gap updated to RESOLVED, correcting its outdated premise along
the way (starting research levels were never actually part of the gap - `ProcessPrimaryTraits`
already had them right, several already live-verified against the real game). Full suite: 197/197,
0 build warnings.

## Waypoints can now target any map entity, not just stars - plus a real star/fleet tap-ambiguity bug it exposed, a "Measure Distance" tool, a global scroll-fling fix, and enemy fleet reports finally showing something (2026-09-17)

The user asked to add fleet orders that can target other fleets, not just stars - the Inspector's
"Add Waypoint" destination picker only ever offered `DestinationOptions` (every known star's
name). Removed the dropdown (and the "Add Waypoint" button that used it) entirely, per the user's
own direction ("adding waypoints only via tapping on other entities on the map") - the Warp/Task
fields and the existing "Add via Map Tap"/"Cancel Map Tap" pair are now the only way to add one.

**Generalized map-tap targeting.** `SelectionService.TryConsumeWaypointTarget` previously matched
only `Star`/`StarIntel`. It now resolves any `Mappable` (every selectable thing on the map already
derives from this: `Star`, `StarIntel`, `Fleet` - own or another empire's - `FleetIntel`,
`Minefield`, `Wormhole`) to a raw `Position` plus a best-effort `Destination` star name (blank for
anything not sitting at a known star - the same "plain deep-space waypoint" shape already used
elsewhere in this codebase; a fleet in orbit resolves to that star's name via `Fleet.InOrbit`,
since arriving there should register as visiting it same as a star-targeted waypoint would).
`ArmWaypointTarget`'s callback signature changed from a bare star-name `string` to `(NovaPoint,
string)` to carry this. `InspectorViewModel.AddWaypoint` now takes that position/destination
directly instead of resolving a picked name through `StarReports`.

**Real bug found and fixed while verifying this: a star sitting under (or very near) one of its
own fleets was nearly unclickable.** Both star and fleet markers share an identical, deliberately-
oversized 32x32 (16px-radius) invisible tap target around their much smaller visible icon (already
documented in-line in `StarMapDocumentView.axaml` as intentional, to make markers reachable at any
zoom level) - but fleet markers are declared after (so rendered on top of) star markers in the same
Panel, and a fleet "at" a star shares that star's exact position. Any tap within that shared 16px
radius always resolved to the fleet, regardless of whether the tap point was actually closer to
the star - reported independently by the user ("clicking on the star is almost impossible - it
typically defaults to the ship") while this same session was verifying the new fleet-tap-waypoint
feature. Fixed with real nearest-wins arbitration: `StarMapDocumentViewModel.
FindNearestStarOrFleetMarker(x, y)` picks whichever star/fleet marker's center is actually closest
to the tap point, among only those whose own 16px radius contains it. Wired in via a Tunnel-phase
`PointerPressed` handler on the map's root Panel (`StarMapDocumentView.axaml.cs`) - Tunnel, not the
default Bubble a plain per-marker event handler would use, since a `Button` marks its own internal
press handling `Handled` during the tunnel phase before ever bubbling back out to a handler
attached to that same Button (confirmed earlier this session on Ship Design's component buttons).
Firing on the shared ancestor, ahead of that swallowing, lets this resolve the tie before either
marker's own Button click fires; each marker's own `Command="{Binding SelectCommand}"` binding
stays in place as a fallback for taps outside this radius (e.g. a minefield, whose own much larger,
size-driven hit area and lower z-order already worked correctly and is untouched by this fix).
**Verified live**: tapping dead-center on Nova (home star + Scout + Starbase, all at the identical
position) now selects "Nova (Planet)" instead of a fleet.

**"Measure Distance" tool**, requested mid-session: a `Measure` button (next to the zoom controls)
arms the same tap-to-pick gesture, using whatever's currently selected as the "from" point -
disabled unless that's a real `Mappable`. The next tap resolves via
`SelectionService.ArmMeasureTarget`/`TryConsumeMeasureTarget` (a simpler sibling of the waypoint
version - any `Mappable` at all satisfies it, using its own `Position`/`Name` directly, since
measuring doesn't need the waypoint case's turn-processing-relevant star-name logic) to a result
banner: "{from} to {to}: {distance:0.0} ly", dismissible or replaced by the next measurement -
cleared automatically whenever the selection itself changes, so a stale readout never lingers.
Arming either the waypoint or the measure gesture cancels the other, since only one banner/gesture
makes sense on the map at a time. **Verified live**: "Nova to Diddley: 63.2 ly", consistent with
the ~1.8-year warp-6 ETA already shown for a real route between the same two stars.

**Global scroll-fling fix.** The user noted the "tap does nothing on the first try after a scroll"
issue fixed earlier today only on Ship Design's own ScrollViewer was "common on many controls."
Moved the fix (`IsScrollInertiaEnabled="False"`) into a single `Style` in `App.axaml` targeting
every `ScrollViewer` in the app, and removed the now-redundant local override on Ship Design's -
one fix, every scrollable panel, instead of finding and patching each of the 14 files with a
ScrollViewer individually.

**Enemy fleet reports now show something.** The user reported tapping a foreign fleet showed
"Nothing selected" - `InspectorViewModel.Refresh`'s switch had a case for every selectable kind
(`Star`, `Fleet`, `StarIntel`, `Minefield`) except `FleetIntel`, so it fell all the way through to
the `default: Name = "Nothing selected"` branch, discarding whatever the report actually knew.
Added `ShowFleetReport(FleetIntel report)`: Owner (resolved to a race name via
`EmpireReports`/`RaceName` where known, matching `PlayerRelationsViewModel`'s own pattern), report
age, ship count/mass (only ever set from `ScanLevel.InScan` upward per `FleetIntel.Update` - shown
as "Unknown" rather than a stale zero when the report has nothing more than a name/position/owner),
speed, in-orbit, and starbase-type flag. Also, per the user's own follow-up ("should also work for
enemy fleets orbiting a star"): `ShowStar`/`ShowStarReport`'s existing "orbiting fleets" list
(previously only ever populated from this empire's own `OwnedFleets`) now also includes any
`FleetReports` entry sitting at that star's position via a new shared
`BuildForeignOrbitingFleetRows` helper - matched by position rather than a star-name reference,
since `FleetIntel.InOrbit` is only a bool (whether it's at some star at all, with no reference to
which one). Not live-verified (the session's reusable test save has no other empires - "No other
empires known yet" on Player Relations) - confidence rests on code review and the same
already-proven `EmpireReports`/`OrbitingFleetRowViewModel` patterns used elsewhere, an honest gap
disclosed rather than glossed over.

**Build/test**: full suite 197/197, 0 build warnings.

## Investigated a reported Auto Build "stuck gray" bug (turned out to be correct, if confusing, real Stars! behavior) - plus a new "Merge With Fleet" waypoint task (2026-09-17)

**Auto Build investigation.** The user reported: a star's auto-build Factories/Mines queue rows
were "working fine," then a population reduction (below what the star could operate) made them
stop, and after population recovered they stayed gray instead of resuming - only manually raising
the row's Quantity made them active again. Traced this all the way through
`ProductionCompletionEstimator.Estimate` (`autoBuildAlreadySatisfied = currentCount >= order.
Quantity` → Gray), `ProductionOrder.Process` (an auto-build order's Quantity is a *target to
maintain*, never auto-decremented, never auto-raised), and `ProductionViewModel`/
`ProductionItemViewModel` (Quantity is set once, whatever the player typed at creation or via the
+/- buttons - nothing anywhere recomputes it from `Star.GetOperableFactories()`/
`GetOperableMines()`, at creation or on any later turn). **Conclusion: this is not a bug - it's the
real, documented Stars! "Auto Build Up to N" mechanic**, confirmed against
`docs/behavior-specs-4/production-queue.md` §9's own primary-source citations: the manual's own
production templates list `Factories (Auto Build) Up to 10` *and*, separately, `Up to 25` - two
distinct entries at different targets, because the target is meant to be manually bumped as a
colony matures, not auto-tracked. Example 1 in that same doc explicitly states "auto-build items
never overshoot their stated count" and "an auto-build order going idle (not deleted) once its
target is met" as intended behavior. Since `Factories`/`Mines` counts are never reduced by a
population drop (only bombing destroys them - confirmed via `Bombing.cs`, the only place either
counter is ever decremented), a row already at its fixed target before the population dip stays at
that same target after recovery, regardless of population - matching exactly what was reported.
**Real, if minor, finding from the investigation**: nothing in the UI explained this "target to
maintain, not a batch" semantic at all - the existing tooltip only described the skip-when-
unaffordable behavior. Expanded it to spell out the actual mechanic and why a row goes gray. No
engine change - the "Auto Build" mechanic itself is correct and matches the original.

**New waypoint task: "Merge With Fleet".** Separately requested: waypoint tasks should support
more of what the engine already implements, including merging with a fleet (`SplitMergeTask`
already existed as a class, just never exposed through the generic Task dropdown - only through
the dedicated Split/Merge tab, which needs a composition editor CargoTask/SplitMergeTask were both
originally deferred for). Added it as a `TaskOptions` entry; picking it and then tapping a fleet
(via "Add via Map Tap" - see the entry above) builds a `SplitMergeTask` with empty Left/
RightComposition and the tapped fleet's own Key as `OtherFleetKey` - `SplitMergeTask.Perform`'s
actual merge path never reads those two dictionaries at all (only a split does), so this is a
complete, real "merge everything" order once the two fleets are at the same position, not a
placeholder. (Remote Mining, also requested, needs no waypoint task at all - confirmed via
`RemoteMiningStep.cs` that it's fully automatic for any fleet with mining equipment sitting in
orbit, own star or not; nothing to add there.)

**A significant, previously-unknown limitation surfaced while verifying this: an orbiting fleet
has no map marker of its own at all**, by deliberate design ported from the original client
(`StarMapDocumentViewModel`'s own comment: "an orbiting fleet is implied by the star it's sitting
on" - only a fleet actively in transit between stars gets a triangle icon). This meant "tap a
fleet to merge with it" only ever worked for a fleet mid-flight - not the far more common case of
two fleets sitting together at a rally point or home star, which was the whole reason the earlier
"nearest-wins" star/fleet tap fix (see the entry above) didn't actually help here: there was never
a competing fleet marker to resolve a tie against at Nova, only the star. Fixed by resolving a
STAR tap (while "Merge With Fleet" is armed) to whichever other owned fleet is in orbit there
(excluding this waypoint's own fleet and the star's starbase) - unambiguous and used directly when
exactly one candidate exists; a specific rejection message ("Diddley has 2 other fleets in orbit -
use the Split/Merge tab...") when there's more than one, rather than guessing. Also refined the
map's own nearest-wins tie-break itself: unarmed, a star still wins a tie (the original bug this
session fixed), but while a waypoint or measure-distance gesture is armed, a FLEET now wins instead
- since arming that gesture at all means the player is very likely aiming for a specific ship, not
the star it happens to be at. **A real, separate bug found while verifying the star-resolution
fallback**: excluding a star's own starbase by checking `fleet.Type == ItemType.Starbase` alone
undercounted correctly on a fresh game but double-counted a starbase as a phantom "other fleet" on
this session's own long-lived test save, whose starbase predates the `AllocateStarbase` Type-
stamping fix from earlier today and still carries the old, uncorrected `Type=Fleet` value. Fixed by
also excluding by direct reference against the owning `Star.Starbase`, which is correct regardless
of what `Type` says.

**Verified live, repeatedly**, on the same `Nova_Test` emulator session: selecting "Merge With
Fleet" and tapping a star with no other owned fleet, exactly one, and more than one all produced
the correct outcome (rejected / real "Merge Fleet" waypoint added with 0.0yr ETA / rejected with
the specific "N other fleets" message respectively) - the Orders list correctly showed "warp 6,
Merge Fleet, 0.0 yrs, 50mg" for the successful case. Full suite: 197/197, 0 build warnings (this
feature's logic lives entirely in Avalonia ViewModels, so - matching this session's established
practice - it's verified live rather than via `Tests.csproj`, which has no project reference to
`Nova.Avalonia`).

## Fixed a reported crash: End Turn, then crash-on-every-load (2026-09-17)

**Reported**: the user's live game crashed on End Turn, then crashed and closed on every
subsequent attempt to load that same save - with no way to even reach the in-app "Share Error
Log" menu, since the crash happened before the game got far enough to render it.

**Diagnostic gap found and fixed first**: grepping `Nova.Avalonia.Android/` turned up zero matches
for `UnhandledException`/`AppDomain`/`AndroidEnvironment` - unlike `Nova.Avalonia.Desktop/
Program.cs`, which already logs any unhandled exception to `nova-avalonia-crash.log`, the Android
host had **no global crash handler at all**, so a genuine crash (as opposed to a deliberate
`Report.Error`/`Report.FatalError` call, which already gets logged) left literally no trail.
Ported the exact same pattern to `Nova.Avalonia.Android/Application.cs`, additionally wiring
`AndroidEnvironment.UnhandledExceptionRaiser` for exceptions crossing the JNI boundary - both now
log to `nova-avalonia-crash.log` on external storage (`GetExternalFilesDir`, reachable via a plain
`adb pull`, no root/`run-as` needed), registered in the constructor so it's live before any of
`OnCreate`'s own setup could crash.

**Root cause found by code review** (this session's own test save has no scenario that reproduces
it, so this wasn't live-verified against the user's actual save - see below): this session's
brand-new "Merge With Fleet" waypoint task (added just before this report) always passes empty
`LeftComposition`/`RightComposition` dictionaries to `SplitMergeTask`, since the real merge path
never reads them. But `SplitMergeTask.Perform` only takes that merge path if the target fleet
(`OtherFleetKey`) can still be found in `sender`/`receiver.OwnedFleets` *at execution time* - if it
can't (e.g. the target was already merged/scrapped/destroyed by an earlier order the same turn),
it silently fell through to the **split** path instead, calling `MakeNewFleet` + `ReassignShips`
with those same empty dictionaries. `ReassignShips` loops over `LeftComposition.Keys` - empty, so
it moves nothing - leaving a genuinely empty-`Composition` fleet, which then got added straight to
`OwnedFleets` via `ServerData`'s "get fleets out of limbo" step with no check that it actually has
any ships. `Fleet.Icon`'s getter already handles an empty `Composition` safely (catches and
returns `null`), but `FleetIntel.ToXml` - called via `AddOrUpdateFleet` → `GenerateReport` the
moment that phantom fleet is added - did an unguarded `Icon.Source`, throwing
`NullReferenceException` **at save time**: the End Turn crash. Whatever then re-triggers a save or
re-generates that same report on load (this codebase's own turn/report regeneration) hit the exact
same null `Icon` again - the crash-on-every-load.

**Fixed at both the trigger and the unguarded null-deref**:
- `SplitMergeTask.Perform` (`Common/Waypoints/SplitMergeTask.cs`): restructured so `OtherFleetKey
  != 0` (a merge was requested) never falls through to the split path - if the target can't be
  found, the order is simply skipped (matching how other "can't do this right now" waypoint
  outcomes are already handled elsewhere), instead of fabricating a fleet with no real composition
  data.
- `FleetIntel.ToXml`/`FleetIntel(XmlNode)` (`Common/DataStructures/FleetIntel.cs`): a null `Icon`
  now saves as an empty element instead of throwing; loading back skips the icon lookup entirely
  when that element is empty, rather than calling `GetIconBySource("")` (which can't parse an
  empty string and would hit the load loop's own `Report.FatalError` → `Environment.Exit` -
  trading one crash for another kind of forced-exit). This is deliberate belt-and-braces: a
  zero-ship fleet could still exist transiently for other reasons (e.g. the one-turn window before
  `ServerData.CleanupFleets` next runs), independent of whatever created this one.

**New regression tests** (none existed for `SplitMergeTask` or this `FleetIntel` path before):
`SplitMergeTaskMissingTargetTest` (a missing merge target creates no phantom fleet and leaves the
original untouched; a real target still merges normally) and `FleetIntelEmptyCompositionTest` (an
empty-`Composition` report saves and reloads without throwing, with `Icon` staying `null` rather
than a bogus lookup result). Full suite: 201/201, 0 build warnings. Not live-verified against the
user's actual save (this session's own test save doesn't reproduce the missing-target scenario);
built into a new Release APK for the user to retry with, which will now also produce a retrievable
`nova-avalonia-crash.log` if any *other*, still-unknown cause turns out to be involved.

**Follow-up: the fix above didn't resolve the user's actual crash** - it still crashed entering the
save via Continue on the rebuilt APK. Since `OpenGameViewModel.Continue()` already wraps
`GameSession.Load`/`GameOpened?.Invoke` in a try/catch that reports failures as a status message
rather than crashing, whatever's actually throwing here must be escaping that catch entirely -
e.g. on a different thread, or during a deferred UI render pass triggered by the screen
transition rather than synchronously inside the Invoke call - so this is a distinct, still-
unconfirmed cause, not the SplitMergeTask path above. The user has no adb access, which blocked
retrieving `nova-avalonia-crash.log` even with last entry's logger in place.

**Added a no-adb diagnostics escape hatch** rather than continue guessing blind: a "Share Crash
Log" and "Share Save File" button, small and understated, on the startup screen itself (`OpenGameView.axaml`/`OpenGameViewModel.cs`) - the one screen confirmed to still load fine even
when Continue/Open is what crashes. Mirrors the existing in-game burger menu's `ShareErrorLog`/
`ShareGameSaveCommand` (`MobileMainViewModel.cs`), which are unreachable in exactly this scenario
since they live inside the view a crash-on-load never reaches. New `PlatformHooks.ShareCrashLog`
hook, wired in `Nova.Avalonia.Android/Application.cs` to read `nova-avalonia-crash.log` and fire
the same native Share-sheet path `ShareErrorLog` already uses; "Share Save File" reads the
continuable game's `.intel` file directly via the `(GameFolder, RaceName)` `FindContinuableGame`
already resolved, so it works even when loading itself is what's failing - no need to successfully
open the game first. Live-verified on the `Nova_Test` emulator: both buttons render on the startup
screen, "Share Crash Log" correctly reports "No crash log found" when nothing has crashed yet, and
"Share Save File" opens Android's native Share sheet with the real `.intel` XML content, no crash.
Full suite: 201/201, 0 build warnings.

Sent this new Release APK to the user (they have no adb, so it has to be sideloaded manually) -
next step is getting either a crash log or the actual corrupted save's XML back from them, since
code review alone hasn't reproduced or confirmed the real cause yet.

**Follow-up 2: "Share Save File" did nothing on the user's real save.** Root cause: `ShareText`
(shared by `ShareErrorLog`/`ShareCrashLog`/the save-sharing commands) stuffed the whole content
into a plain `Intent.ExtraText` string - which silently fails once it crosses Binder's ~1MB IPC
transaction limit. `StartActivity` itself doesn't throw; the receiving chooser just never actually
opens, so nothing visibly happens - exactly what got reported. This project's own test save
(~95KB) never came close to that limit, which is why the earlier live verification of these same
buttons didn't catch it; a real, longer-played game's `.intel` file plausibly does. **Fixed** by
switching `ShareText` (`Nova.Avalonia.Android/Application.cs`) to write the content to a file
under `cache/share/` and hand the receiving app a `content://` Uri via `AndroidX.Core.Content.
FileProvider` instead of an inline text extra - no size limit either way. Needed a `<provider>`
entry in `AndroidManifest.xml`, a new `Resources/xml/file_paths.xml` scoping it to just that
cache subfolder, and an explicit `Xamarin.AndroidX.Core` package reference (already present
transitively via `Avalonia.Android`, pinned explicitly since it's now used directly). Also wrapped
the whole method in a try/catch that logs any failure to the ordinary error log instead of the
previous code, which could never report a failure at all. **Live-verified on the `Nova_Test`
emulator against a real, ~2MB test file** (padded well past the 1MB threshold that broke it) -
confirms the fix, not just that the small-file path still works. Root cause of the user's actual
crash (why no crash log was written) is still open - `StackOverflowException` (uncatchable by
design in modern .NET, terminates immediately) or a native/AOT crash are the leading remaining
hypotheses, both of which would explain a real crash producing zero managed-exception trail. New
Release APK built and sent to the user; next step is the "Share Save File" button (now actually
working) getting me the real corrupted save content to inspect directly.

**Follow-up 3: the shared save file showed nothing wrong - because it was missing the file that
actually matters.** The user sent their `.intel` (client-side "what I know" report) via the fixed
Share Save File button; a full read found no empty Composition, no SplitMergeTask at all, no
dangling fleet/starbase keys, well-formed XML start to finish. Reproducing their exact `.intel` on
the `Nova_Test` emulator (via Open Game) loaded and rendered it with no crash, and End Turn just
submitted normally - which turned out to be because there was no `.sstate` file alongside it at
all (Open Game's Android file-picker copies only the single selected file into a solitary temp
folder - confirmed by reading `OpenGameView.axaml.cs`'s `BrowseButton_Click`), so `TurnHost.
LoadServerState` found nothing and silently no-opped. That's the real gap: `TurnHost.cs` (this
port's in-process replacement for the original's separate host/AI subprocess loop) does all actual
turn advancement - `TurnGenerator.Generate()`, plus any in-process AI's own `DoMove()` - against a
single `*.sstate` file per game folder, which is the *authoritative* state covering every empire,
not just the player's own knowledge. "Share Save File" only ever read the `.intel`, so it could
never have shown a bug in turn processing itself - confirmed by creating a genuine local game via
New Game (with a real `Default AI` opponent, so a real `.sstate` exists) and finding it right there
in the game folder (`Games/<name>/<name>.sstate`) alongside the per-race `.intel`/`.cstate` files.

**Fixed**: added `GameSession.BuildShareableSaveText(gameFolder, raceName)` - bundles the `.intel`
plus whichever single `*.sstate` file exists in that folder (globbed the same way `TurnHost.
LoadServerState` already does, not by assuming a name), each under a clear `===== filename =====`
header, skipping either that's missing rather than failing. Both `OpenGameViewModel.
ShareContinuableSaveCommand` (startup screen) and `MobileMainViewModel.ShareGameSaveCommand`
(in-game burger menu) now call this instead of reading just the `.intel` directly. Live-verified
on the emulator: created a real game via New Game with a Default AI opponent, confirmed
`Games/Feel the Nova/Feel the Nova.sstate` exists on disk, tapped Share Save File, and confirmed
the shared text (273KB, up from ~95KB for the `.intel` alone) contains both `===== Nucleotid.intel
=====` and `===== Feel the Nova.sstate =====` sections. Full suite: 201/201, 0 build warnings. New
Release APK sent to the user - this is what should finally surface the state that was actually
live when End Turn crashed.

**Meanwhile, the user offered direct adb access**: plugging their physical device (the one that
actually reproduces the crash) into this same machine, so real-time `adb logcat`/`adb pull`
debugging becomes possible instead of working from emulator reproductions and shared text dumps.
Far more reliable than continuing to guess from static file review - once connected, next step is
watching logcat live through an actual repro on their device.

**Follow-up 4: exhaustively reproduced the user's exact real data server-side and in the full
live app - found nothing.** With both real files in hand, ran three escalating checks, each
against their actual data (a temporary, since-deleted `ReproUserCrashTest.cs` + copies of their
`.sstate`, never committed): (1) `ServerData.Restore()` (the same load+cross-reference-fixup path
that runs on every load/End-Turn check) - no throw; (2) `TurnGenerator.Generate()` (the actual
turn-advance logic) against their restored state - no throw; (3) `DefaultAi.DoMove()` for the
Humanoid empire, using its own `EmpireData` block extracted straight out of the same `.sstate` and
re-wrapped as a synthetic `.intel` (the AI reads its own `.intel` from disk, which "Share Save
File" never captures) - no throw. All three passed clean.

Went further and reconstructed the *entire real scenario live in the app* on the `Nova_Test`
emulator: placed the user's actual `r.intel` and `Feel the Nova.sstate` at the exact internal path
the `.sstate`'s own embedded `GameFolder` field expects (discovered along the way: `ServerData.
Restore()` never re-points `GameFolder`/`StatePathName` to wherever the file was actually loaded
from - it trusts what's stored in the XML - so a relocated `.sstate` silently sends `TurnHost`'s
per-player file I/O to a stale path; harmless for a real user who only ever plays via Continue
from its original location, but had to be worked around to get a faithful repro here), synthesized
the AI's own `Humanoid.intel`/added it to that same folder, and pointed nova.conf's `Continue`
entry at it directly (bypassing Open Game's Android file-picker, which - confirmed earlier this
session - only ever copies the single selected file, never proves useful for reconstructing a full
multi-file game folder). Result: Continue loaded the map fine, the AI ran and wrote its own orders
in the background exactly as designed, and **End Turn actually advanced the turn cleanly** -
turn-advance messages (tech level up, new component, production report), no crash, process stayed
alive throughout.

This is about as thorough as reproduction from data alone can get, and it came back clean across
the board - strong evidence this isn't a deterministic logic bug triggered by the save's content,
but something environment-specific to the user's actual device (memory pressure on real hardware
the emulator doesn't reproduce, an OS/manufacturer quirk, or a timing-dependent case only real
hardware hits). This is exactly why the user's offer to plug in the actual failing device and get
real `adb logcat` access is the right next step, rather than continuing to guess from data. All
temporary repro files (test + copies of the user's save data) deleted afterward - never committed,
per the "don't retain a user's private save data in the repo" default. Full suite still 201/201
after cleanup.

## Root-caused and fixed the crash, with the user's phone connected directly (2026-09-18)

**Found via live `adb logcat` on the user's actual device**: reproducing Continue on their real
phone (a Samsung Galaxy, arm64) with a Release build showed a **native SIGSEGV** (`Fatal signal
11, fault addr 0x0`) on a binder thread - not a managed .NET exception, which is exactly why the
crash logger from two entries ago never caught anything (`AppDomain.UnhandledException`/
`AndroidEnvironment.UnhandledExceptionRaiser` only observe managed exceptions). A same-scenario
test with a **Debug** build on the same phone died with no crash signal, no exception, no OOM kill
at all in logcat - but `nova-error.log` on external storage told the real story: a
`NullReferenceException` at `Nova.Common.Waypoints.Waypoint..ctor(XmlNode node)`, logged on
*every single load attempt* since 2026-09-17 20:14:35, each one ending "(This error will terminate
the program)". `Waypoint`'s XML loader hits a malformed field (almost certainly an empty
`<Destination/>` with no text child - `mainNode.FirstChild.Value` then throws on the null
`FirstChild` - itself likely left behind by an even earlier crash interrupting a save write) and
calls `Report.FatalError(...)`, which calls `Environment.Exit(1)` - killing the whole app,
forever, since nothing ever repaired the one damaged field. The Release build's SIGSEGV was very
likely just how that same abrupt `Environment.Exit` manifests during an AOT-compiled native
teardown - same root cause either way.

**Fixed** (`Common/Waypoints/Waypoint.cs`): that catch block now calls `Report.Error` (log and
continue - the loop already advances to the next sibling regardless) instead of `Report.
FatalError`, and `Task ??= new NoTask();` was added after the loop, since this XML constructor
(unlike the parameterless one) never otherwise defaults it - a wypoint whose task-shaped child
failed to load would otherwise leave `Task` null and crash the many callers that assume every
waypoint has one.

**Then swept the whole codebase for the same pattern**, since one bad field taking down the
entire app was never specific to `Waypoint.cs` - a background agent audited every other `Report.
FatalError` call site and classified all ~30 of them. Fixed the two categories that matched:
- **18 files** with the identical "per-child-XML-node loop, catch general Exception, call
  FatalError" shape as `Waypoint.cs` (`ClientData.cs`, `Bomb.cs`, `Component.cs`, `Engine.cs`,
  `EmpireIntel.cs`, `FleetIntel.cs`, `Star.cs`, `Message.cs`, `Orders.cs`, `Intel.cs`, `Race.cs`,
  `RaceIcon.cs`, `Resources.cs`, `RaceRestriction.cs`, `ScoreRecord.cs`, `ServerData.cs`,
  `ShipIcon.cs`, `StarIntel.cs`) - all downgraded to `Report.Error` the same way. `ServerData.cs`
  needed one extra fix: its sibling-advance sat *inside* the `try` (unlike every other file's,
  which sit after it), so downgrading naively would have spun the loop forever on a node that
  kept throwing instead of skipping it - added the same advance to the `catch` too.
- **`EmpireData.cs`'s own XML constructor had no `try`/`catch` around its per-field switch at
  all** - a malformed field there threw straight out uncaught, arguably worse than the
  `Report.FatalError` pattern since not even a controlled exit happened. Added one, plus a second
  around its separate `LinkReferences()` call (the cross-reference resolution pass with the
  unguarded `Designs[token.Design.Key]`-style indexers found earlier this session) - one
  legitimately mismatched reference no longer throws out of the whole constructor.
- **3 structural "node is null"/"could not find X node" checks** (`Item.cs` x2, `Mappable.cs` x2,
  `BattleStep.cs` x2) - confirmed each already has a `return;` immediately after the FatalError
  call, so downgrading to `Report.Error` is a pure win (same blank-object outcome, minus the
  process exit).
- Deliberately left as fatal (Bucket B, not touched): genuine top-level "the app cannot start/
  continue at all" preconditions in `ClientData.Initialize()`, `IntelReader.cs`, `AllComponents.cs`,
  `FileSearcher.cs`, and the WinForms launcher/console subprocess-launch failures - these gate
  whether the whole app can function, not one field in one save.

**New regression test** (`Tests/UnitTests/WaypointMalformedFieldTest.cs`): an empty `<Destination/>`
element no longer throws and leaves every other field (WarpFactor, Task) intact; a Waypoint with
no task-shaped child at all still defaults `Task` to `NoTask()` rather than null. Full suite:
203/203, 0 build warnings.

**Confirmed fixed live, on the user's actual device, with their actual corrupted save**: installed
a Debug build with this fix, and `nova-error.log` showed the exact same `NullReferenceException`
at the exact same call site - but this time logged as "Nova has encountered an error, but will
continue anyway" instead of "(This error will terminate the program)". The user confirmed the game
loaded successfully afterward and the map is visible and usable again. This is the real fix - not
just a diagnostic aid - for a crash that had made the user's save completely unloadable for over
12 hours across at least 11 separate attempts.

Next: build and ship a Release APK with this fix (the Debug build the user is currently running
was for diagnosis only), and consider whether the underlying data corruption (the malformed
Waypoint field itself) is worth a proactive repair pass, or whether leaving it silently skipped
(current behavior) is sufficient.

## Fixed a real ANR: reordering a production queue item froze the whole app (2026-09-18)

**Reported live**: moving a stardock (a permanently-blocked, "Red" item - insufficient minerals,
would never complete) up past a long run of factory orders froze the app entirely; the process
stayed alive (confirmed via `adb shell pidof`) rather than crashing.

**Diagnosed live via `adb logcat`** while it was still frozen (the user's phone was connected
throughout): `InputDispatcher`/`ActivityManager` both logged a genuine ANR ("Input dispatching
timed out ... Waited 10000ms for MotionEvent"), and critically the CPU-usage snapshot in that same
ANR report showed the app pegged at ~100%, almost entirely user-mode, for the full window - not a
deadlock (which reads as near-0% CPU while blocked on a lock), a real, ongoing computation.

**Root cause**: `ProductionViewModel.RebuildQueueRows` - called after every single queue edit,
including a reorder - called `ProductionCompletionEstimator.Estimate` once per row to decide each
row's color/years-to-finish. Each call ran its own fully independent `Simulate`: a full XML
clone of the star plus up to 100 simulated years re-running the *entire* queue's `UpdateMinerals/
UpdateResearch/UpdateResources/UpdatePopulation/Process` cycle from scratch. Since every row's
simulation processes the identical queue from year 1 regardless of which row is being tracked, N
rows meant N-fold *entirely redundant* resimulation of the same 100 years - moving one item in a
"several-dozen-line queue full of factories" meant several dozen full reclone-and-resimulate
passes, synchronously on the UI thread, on every single reorder. Comfortably enough to blow past
Android's 10-second ANR watchdog on real hardware, even though it would eventually have finished
(this was slow, not literally infinite).

**Fixed** (`Common/Production/ProductionCompletionEstimator.cs`): added `EstimateAll(star, race,
researchBudget)`, which runs *one* shared simulation pass tracking every row's (start, finish)
simultaneously, instead of one independent pass per row - O(1) full-star-clones instead of O(N).
The original per-index `Estimate(star, index, ...)` is now a thin wrapper (`EstimateAll(...)
[index]`) kept for exact backward compatibility with its existing callers/tests. `ProductionViewModel.
RebuildQueueRows` now calls `EstimateAll` once per rebuild instead of `Estimate` once per row.

**New regression test** (`EstimateAll_MatchesCallingEstimatePerIndex`): pins `EstimateAll`'s
per-row results against the original per-index `Estimate` for a mixed queue (a satisfied
auto-build alongside a permanently-blocked manual order, mirroring the user's own report) - the
optimization can never silently change what a row shows. Full suite: 204/204, 0 build warnings.
Shipped in the same Release build as the waypoint-destination fix above, installed directly onto
the user's connected phone.

**Follow-up: that fix wasn't the (whole) story - the app still froze on the very next attempt.**
Caught it live: `adb shell pidof` showed the process still alive, `adb shell top` showed it
pegged at 96.5% CPU *right then*, and a `kill -3` (to grab a Mono-runtime stack dump while it was
still running) landed just after the user had already force-closed it - but Android's own ANR
handler had *already* written a full thread dump to the dropbox before that (`adb shell dumpsys
dropbox --print data_app_anr`), which survived independently. The main thread's own native frame
was an unsymbolized address inside `libaot-Nova.Common.dll.so` - this codebase's own game-logic
assembly, actively executing, not blocked on a lock or waiting on I/O (matching the CPU numbers:
a real spin, not a deadlock).

**Actual root cause**: `ProductionOrder.Process()`'s inner `while (Quantity > 0) { if (Unit.
IsSkipped(star)) break; if (Unit.Construct(star)) { ... } }` loop assumes `IsSkipped()`/
`Construct()` jointly guarantee forward progress every single iteration - either the unit gets
skipped (break) or a whole unit completes (Quantity--). Neither is actually guaranteed:
`FactoryProductionUnit.Construct()`'s partial-build branch computes how much of `remainingCost` a
star can afford this year and subtracts that from both `remainingCost` and `star.ResourcesOnHand`
via `Resources`' own `* double` operator (`Common/DataStructures/Resources.cs`), which rounds via
`Math.Ceiling` on each of the four `int` fields independently - a genuine rounding edge case here
can leave a resource just above zero (so `IsSkipped` - a plain `<= 0` check - never fires) while
the unit still isn't affordable (so `Construct` never returns true either) - and if that leftover
amount doesn't move on the next call, *nothing about the state ever changes again*: a pure,
100%-CPU infinite loop with no natural exit, entirely within a single `Process()` call for a
single simulated year. This a completely different bug from the redundant-resimulation one just
above - fixing that one made the *normal* case dramatically cheaper, but this stuck state was
already a true infinite loop, immune to any amount of resimulation-count optimization.

**Fixed** (`Common/Production/ProductionOrder.cs`): `Process()` now snapshots `star.
ResourcesOnHand` before each `Construct()` call and bails out of the while loop for this year if
a `false` result didn't actually change anything - a tick that neither completes a unit nor
spends any resources at all is never going to resolve itself just by being asked again. Applied
to both of `Process`'s while loops (the ordinary manual-order one and the auto-build "maintain up
to N" one), since both have the identical shape/risk.

**New regression tests** (`Tests/UnitTests/ProductionOrderProcessTest.cs`): a minimal
`IProductionUnit` test double whose `IsSkipped`/`Construct` reproduce the exact stuck condition
directly (always false, never spends anything) - confirms `Process()` terminates instead of
hanging, for both the manual and auto-build branches, with a quantity of a million to make
absolutely sure a bound exists. Full suite: 206/206, 0 build warnings. Shipped in the same
Release build cycle, installed directly onto the user's phone once again - this is the fix
expected to actually resolve the freeze, not just make the common case faster.

## Found and fixed the actual source of the corrupted waypoint data (2026-09-18)

The crash-hardening fix above stopped the app from dying, but the same `NullReferenceException`
kept reappearing in `nova-error.log` on every subsequent load/End-Turn (2 occurrences per load,
then 6 after one End Turn) - the malformed data itself was still there and, worse, still being
*produced*, not just replayed. Extended `GameSession.BuildShareableSaveText` to also bundle the
player's own `.cstate` and every `*.orders` file in the game folder (not just `.intel`/`.sstate`,
which had already come back clean) - a `.cstate` holds the client's own in-progress waypoint
edits, and `.orders` is what `OrderReader.ReadOrders()` re-parses on every single load/End-Turn,
matching a fault that recurs *more than once per turn* exactly as reported live.

Got the user to re-share via the same button, and found it immediately in `r.orders`: two
`<Command Type="Waypoint">` entries with a genuinely empty `<Destination>` element (whitespace
only between the tags - `XmlDocument` doesn't preserve insignificant whitespace by default, so
this parses back with no child node at all, exactly the shape that made `Waypoint`'s `mainNode.
FirstChild.Value` throw). Both were `Add` waypoints targeting a *position* rather than a star -
matching this session's own earlier "Merge With Fleet"/generalized-map-tap work exactly.

**Root cause, confirmed**: `SelectionService.TryConsumeWaypointTarget` (`Nova.Avalonia/
ViewModels/SelectionService.cs`) - added earlier this same session as part of generalizing
waypoint targeting from "stars only" to "tap anything on the map" - built its best-effort
`Destination` label with `fleet.InOrbit?.Name ?? string.Empty` for a tapped Fleet, and a literal
`string.Empty` outright for FleetIntel/Minefield/Wormhole targets. Its own comment claimed this
matched "how a plain deep-space waypoint already works everywhere else in this codebase" - but
that's wrong: every other place that builds this exact label (`TurnGenerator.cs`, `StarMap.cs`,
`FleetReport.cs`, `FleetReportRowViewModel.cs`) uses `"Space at " + position`, never an empty
string. So tapping a fleet that wasn't in orbit, a foreign fleet's report, a minefield, or a
wormhole as a waypoint target - a completely ordinary, correct thing to do in this session's own
"tap anything on the map" feature - silently produced a save-corrupting empty Destination every
time.

**Fixed**: those four cases now use the same `"Space at " + position` fallback as everywhere else,
matching the established convention instead of inventing a new (broken) one. This is the real fix
- the crash-hardening from the previous entry was necessary regardless (many other things can
still corrupt a field, and one bad field should never take down the whole app), but this is what
actually stops new empty Destinations from being created in the first place.

Verification: `dotnet build`/`dotnet test` both clean (203/203) - `SelectionService.cs` lives in
`Nova.Avalonia`, which `Tests.csproj` has no project reference to (an established, pre-existing
gap in this repo, not new), so this needs live verification on-device/emulator rather than a unit
test, matching how this session's earlier `SelectionService` changes were verified. Not yet
live-verified as of this entry - next step is tapping a fleet in deep space (not orbiting a star)
as a waypoint target and confirming the resulting order's Destination reads "Space at (x, y)"
