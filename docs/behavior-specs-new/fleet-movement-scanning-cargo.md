...
- **Cargo-pod capacity bonus per pod, and per-hull cargo-pod slot limits**, were not captured in this pass beyond the base hull table above; would need the same tech-browser-style source as the engine table.
