...
A pass over the executable's race-wizard save/finish logic located and substantially reconstructed the point-total calculation this document's Open Questions previously flagged as entirely undocumented. The function computes an internal raw total, which is **divided by 3** at the very end to produce the RW point figure the wizard displays; the race can only be saved once this final divided total is ≥ 0. Recovered structure (raw units, before the final ÷3, unless noted):
...
5. Further quadratic/cubic penalty terms apply to two of the economic-slider fields once either exceeds a value of 10, with an extra multiplier for one specific PRT.
6. ~~**LRT stacking-penalty formula is unknown.**~~ **RESOLVED structurally.** A 14-entry signed per-LRT cost table (matching the 14 LRT checkboxes) is summed across every selected LRT, then an escalating penalty is added on top:
   ...
   The 14 individual per-LRT costs themselves are stored as pure data and were not recoverable from the code alone (no accompanying string/label table survives), so the *schedule* above is confirmed but the *base* per-LRT prices are still unconfirmed — see Open Questions. A further PRT-dependent flat adjustment (distinct values for three specific PRTs) applies for one specific LRT selection beyond this schedule.
7. **Research-cost-class net-bias term**: summing `(costClassValue − 1)` across all six fields (Cheap = −1, Normal = 0, Expensive = +1) gives a net bias; a net-positive bias costs a quadratic `−(bias²) × 130` raw units (plus extra flat corrections at specific bias values), while a net-negative bias instead refunds points via a lookup table — the concrete mechanism behind this document's own guidance to "offset every Cheap field with a matching Expensive field."
...
- **Ultimate Recycling's** 90%-at-starbase / half-that-at-a-bare-planet split is confirmed exactly, plus two new details: the recovered minerals and (separately) a portion of resources are both returned, and the resources specifically become available starting the following year rather than immediately.
...
- ~~**No documented absolute point-cost table.**~~ **PARTIALLY RESOLVED by inspection of the
  exported client** — see §1a above for the reconstructed formula *shape* (baseline, growth-rate
  term, axis-deviation refund, immunity-count penalty, LRT-stacking schedule, research-bias term,
  the AR-specific penalty, and two exactly-confirmed flat costs). What remains unresolved: the
  underlying 10-entry PRT-immunity/base-cost table and 14-entry per-LRT base-cost table are
  confirmed to exist and be referenced by this formula, but their individual numeric values are
  stored as pure data with no accompanying label/string table in the analyzed material, so the
  725/1,448-point immunity figures and the individual LRT prices below remain unconfirmed against
  the binary (though also not contradicted).
- ~~**LRT stacking-penalty formula is unknown.**~~ **RESOLVED structurally by inspection of the
  exported client** — see §1a item 6 above for the exact thresholds and per-step costs. The
  underlying 14 individual LRT base prices are still unconfirmed (see the item above).
